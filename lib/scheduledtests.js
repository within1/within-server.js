...
function scheduledTests() {
	...
	// there should be no unsent notifications in the past
	.then(function() { return models.sequelize.query("SELECT * FROM [dbo].[Notifications] where HasSent = 0 and DateTarget < GETDATE() order by DateTarget asc", { type: models.sequelize.QueryTypes.SELECT})	})
	.then(function(r) {
		if (r.length > 0)
