...
function scheduledTests() {
	...
	*/
	...
	return models.sequelize.query("SELECT * FROM [dbo].[Notifications] where HasSent = 0 and DateTarget < dateadd(hh, -1, GetDate() ) order by DateTarget asc", { type: models.sequelize.QueryTypes.SELECT})
	.then(function(r) {
		if (r.length > 0)
