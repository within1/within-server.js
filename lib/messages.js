		...
		if ( ((msg.length > 0) && (Type != 4)) || (Type == 2) ) {
			...
			.then(function() {
				...
				return notif.SendPushNotification(ruser, 0,
					copytext("PushMessageReceivedCopy1")+" "+cuser["FirstName"]+" "+copytext("PushMessageReceivedCopy2"),
					msgres["MessageID"], cuser["ID"], notif.pushTypes["MessageReceived"]  );
			})
			.then(function() {
				// cancel reminder notification & set up, or cancel "don't leave em hanging" notification
				return lifecycle.MatchMessaged(matchid, cuser["ID"], msgres["MessageID"]);
			})
		}
	})
	.then(function() {
