	...
	return {
		...
		"isFacebookFriend" : function(cb) {
			...
			getFacebookFriends(cuser, function(err, ids) {
				...
				return models.sequelize.query("select ID from Users where FacebookID in ("+sfb+")", { type: models.sequelize.QueryTypes.SELECT})
				.then(function(res) {
					cb(null, res);
				})
			})
...
if (!module.parent) {
	matchUser(4067, 0, function(err, res) {
		console.log(res);
		process.exit(0);
	})
}
