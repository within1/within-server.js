...
var scheduledTests = require("../lib/scheduledtests.js")
...
and appstatus = 2 ", { type: models.sequelize.QueryTypes.SELECT})
	.then(function(res) {
		...
		for (var i in res) {
			...
			updateOps.push(notif.SchedulePushNotification(res[i], nextDate, copytext("PushNewMatchAvailableCopy"), null, res[i]["ID"], cns.pushTypes["NewMatchAvailable"]));
		}
...
var cronRun = function() {
	...
	.then(function() {
		return scheduledTests.scheduledTests();
	})
	.then(function() {
