...
function SendEmailNotification(userinfo, dateCreated, hrsDelay, emailType, otherUserName, imageURL, content, otherUserID) {
	...
	.then(function(textvalues) {
		...
		if (emailSubjects[emailType] !== undefined) {
			...
			resnotif["EmailSubject"] = textvalues("EmailMesssageReceivedSubject")+" "+otherUserName;
		} else if (emailType == emailTypes["TypeEmailThanxReceived"] ) {
			resnotif["EmailSubject"] = textvalues("EmailThanxReceivedSubject1")+" "+otherUserName+textvalues("EmailThanxReceivedSubject2");
		} else if (emailType == emailTypes["TypeEmailSomeoneReferred"] ) {
			resnotif["EmailSubject"] = textvalues("EmailReferredSubject")+" "+otherUserName;
		}
	...
	.then(function(msg) {
		if (emailType == emailTypes["TypeEmailIncompleteOnboarding"]) {
			 ...
			 models.Users.update({"IncompleteOnboardingEmailNotificationID" : msg["ID"]}, { where : {ID : userinfo["ID"]}});
		}
	})
}
...
// Sends and email to TeamWithin that FlaggingUser flagged FlaggedUser
function SendUserFlaggedEmail(flaggedid, flaggedname, flaggingid, flaggingname, totalflags ) {
	return SendAdminMail("FlagEmail", "User Flagged",
			flaggingname+" flagged "+flaggedname+"\n"+
			"Flagging user's ID: "+flaggingid + "\n" +
			"Flagged user's ID: "+flaggedid +"\n"+
			flaggedname+" now has "+totalflags+" flags"
		);
}
