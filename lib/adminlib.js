...
function ProcessUserApplication(userid, newstatus) {
	...
	.then(function(cr) {
		...
		return models.Users.findOne({where : { IsTeamWithin : true} })
			...
			.then(function(tv) {
				textvalues = tv;
				return msglib.AddMessage(wuser["ID"], userid, textvalues.get("WithinTeamFirstMessageCopy"), notif.msgTypes["TeamWITHIN"]);
			})
			...
			.then(function() { return notif.SendPushNotification(cuser, 0,
				textvalues.get("PushYoureApprovedCopy1")+" "+cuser["FirstName"]+" "+textvalues.get("PushYoureApprovedCopy2"), "", userid, notif.pushTypes["YoureApproved"] ) })
	})
}
