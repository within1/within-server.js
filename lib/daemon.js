...
function notifEmailSend(crow) {
	...
	.then(function(cuser) {
		...
		if ((crow["Type"] == notif.emailTypes["TypeEmailMessageReceived"] )) {
			tcontent.push({"name" : "MsgSenderName", "content" : crow["OtherUserName"] })
			tcontent.push({"name" : "MsgSenderImg", "content" : crow["ImageURL"] })
			tcontent.push({"name" : "MsgContent", "content" : crow["PushMessage"] })

			msg["headers"] = {"Reply-To" : "qwdqdqwdqdw"};

		} else if ((crow["Type"] == notif.emailTypes["TypeEmailThanxReceived"] )) {
			tcontent.push({"name" : "ThankerName", "content" : crow["OtherUserName"] })
			tcontent.push({"name" : "ThankerImg", "content" :  crow["ImageURL"]})
			tcontent.push({"name" : "ThanxContent", "content" :  crow["PushMessage"] })
		} else if ((crow["Type"] == notif.emailTypes["TypeEmailSomeoneReferred"] )) {
...
function notifPushSend(crow) {
	...
	.then(function(cuser) {
		...
		note.urlArgs = [crow["SourceTable"]];
		if (crow["UserID"] != null)
			note.urlArgs.push(crow["UserID"]);
		if (crow["MessageID"] != null)
			note.urlArgs.push(crow["MessageID"]);
...
var notifSend = function(rowid, callback) {
	models.Notifications.findOne({where : {ID : rowid }, raw: true })
	.then(function(crow) {
		if (crow == null)
