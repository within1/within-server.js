...
var templates = {
	...
	8 : "no-template-TypeEmailPersonOnWaitlist",
	9 : "new-week-new-recommendation"
};
...
function notifEmailSend(crow) {
	...
	.then(function(cuser) {
		...
		var tcontent = [];
		var msg = {};
		console.log("Email Notification sending: "+cuser["EmailNotificationsOpt"]+" - "+crow["Type"]);
		// check for email sending preferences
		if ( ((cuser["EmailNotificationsOpt"] & 1) == 0) && (crow["Type"] == cn.emailTypes["TypeEmailMessageReceived"] ) )
			return null;
		if ( ((cuser["EmailNotificationsOpt"] & 2) == 0) && (crow["Type"] == cn.emailTypes["TypeEmailThanxReceived"] ) )
			return null;
		if ( ((cuser["EmailNotificationsOpt"] & 4) == 0) && (crow["Type"] == cn.emailTypes["TypeEmailNewWeekRecommendation"] ) )
			return null;

		msg["from_email"] = copytext("WithinFromEmail");
		msg["from_name"] = copytext("WithinFromName");
		...
		tcontent.push({"name" : "EmailAddress", "content" : crow["RecipientEmail"] });
		if ((crow["Type"] == cn.emailTypes["TypeEmailMessageReceived"] )) {
			...
			msg["headers"] = {"Reply-To" : "reply_"+crow["ReplyCode"]+config["emails"][env]["Inbound"]};
			msg["from_email"] = "reply_"+crow["ReplyCode"]+config["emails"][env]["Inbound"];
...
// converts a newmatch push notification to email to be sent
function NewMatchPushToEmail(crow, cuser) {
	crow["Type"] = cn.emailTypes["TypeEmailNewWeekRecommendation"];
	crow["EmailSubject"] = "New week, new recommendation!";
	crow["RecipientEmail"] = cuser["EmailAddress"];
	crow["RecipientName"] = cuser["FirstName"];
	crow["FromName"] = copytext("WithinFromName");
	crow["FromEmail"] = copytext("WithinFromEmail");
	return crow;
}
...
function notifPushSend(crow) {
	...
	.then(function(cuser) {
		...
		if (cuser["ShouldSendPushNotifications"] == false) {
			// Users who have push notifications turned off receive this via email
			if (crow["SourceTable"] == "NewMatchAvailable")
				return notifEmailSend(NewMatchPushToEmail(crow, cuser));
			return null;
		}
...
function notifRefill() {
	return models.Notifications.findAll({where : {HasSent : 0, DateTarget : { $lte : dateFormat( new Date(), "isoUtcDateTime") }}, order: "DateTarget asc" , raw : true } )
	.then(function(cdata){
		if (cdata.length == 0)
			return false;
		for (var i in cdata)
...
var notifSend = function(rowid, callback) {
	console.log("[DEMON] Processing "+rowid+"  notifqueue tasks: "+JSON.stringify(notifqueue.tasks));
	...
	.then(function(p) {
		console.log("Success sending "+rowid+" : "+JSON.stringify(p,0,4));
	})
	.catch(function(e) {
...
module.exports = {"notifRefill" : notifRefill, "templates" : templates };
