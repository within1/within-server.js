...
function getUserModelInfo(json) {
	return copyValues(json, ["FirstName", "LastName", "EmailAddress", "FacebookID", "Gender", "ShouldSendPushNotifications", "ShouldSendEmailNotifications", "Birthday", "DeviceToken"], ["FirstName", "LastName"]);
}
// -------------------------------
			...
			.then(function() {
				...
				.then(function(ids) {
					...
					for (var i in req.body["UserEducation"]) {
						var csch = copyValues(req.body["UserEducation"][i], ["Degree", "StartMonth", "StartYear", "EndYear", "EndMonth", "Description", "Major", "JourneyIndex", "Name"]);
